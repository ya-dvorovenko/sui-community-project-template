...
export default function EventsHistory() {
  ...
  const eventQueries = useSuiClientQueries({
    queries: [
      ...
      },
      {
        ...
        params: {
          query: {
            MoveEventType: `${packageId}::battleplace::HeroBought`
          },
          limit: 20,
          order: "descending"
        },
      ...
      },
      {
        ...
        queryKey: ["queryEvents", packageId, "BattlePlaceCreated"],
        enabled: !!packageId,
      },
      {
        method: "queryEvents",
        params: {
          query: {
            MoveEventType: `${packageId}::battleplace::BattlePlaceCompleted`
          },
          limit: 20,
          order: "descending"
        },
        queryKey: ["queryEvents", packageId, "BattlePlaceCompleted"],
        enabled: !!packageId,
      }
    ]
  });
  ...
  if (isListedPending || isBoughtPending || isBattleCreatedPending || isBattleCompletedPending) {
    return (
      <Card>
              ...
              <Card key={`${event.id.txDigest}-${index}`} style={{ padding: "16px" }}>
                <Flex direction="column" gap="2">
                  <Flex align="center" gap="3">
                    <Badge 
                      color={
                        event.type === 'listed' ? 'blue' : 
                        event.type === 'bought' ? 'green' :
                        event.type === 'battle_created' ? 'orange' : 'red'
                      } 
                      size="2"
                    >
                      {event.type === 'listed' ? 'Hero Listed' : 
                       event.type === 'bought' ? 'Hero Bought' :
                       event.type === 'battle_created' ? 'Battle Place Created' : 'Battle Completed'}
                    </Badge>
                  ...
                  <Flex align="center" gap="4" wrap="wrap">
                    ...
                    {event.type === 'battle_completed' && (
                      <>
                        <Text size="3">
                          <strong>🏆 Winner:</strong> ...{eventData.winner.slice(-8)}
                        </Text>
                        <Text size="3">
                          <strong>💀 Loser:</strong> ...{eventData.loser.slice(-8)}
                        </Text>
                      </>
                    )}
                  </Flex>
